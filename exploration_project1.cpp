...
float getFrontierCost(int frontierId){
  //////////////////////////////////////////////////////////////////////
  /* getFrontierCost takes as argument the id of the frontier that is being
  searched and outputs the value of the cost function for this specific 
  frontier */
  //////////////////////////////////////////////////////////////////////
  
  // Define the frontier center we are dealing with
  geometry_msgs::Point frontier_center = frontiers_[frontierId].free_center_point;
  // Define size of the frontier
  float frontier_size = frontiers_[frontierId].size;
  // Define distance to frontier
  float frontier_dist = hypot(frontier_center.y-robot_pose_.position.y,
                          frontier_center.x-robot_pose_.position.x);
  // Define weights of the cost function
  float weight_frontier_size = 1.0;
  float weight_frontier_distance = 10.0;
  float weight_occupancy_path = 1.0;
  // Value of division determines how many cell's occupancy value will be 
  // taken into account up to the frontier's position. The lowest, the more precise.
  float divisions = 0.1;
  // Define increments for which grid values will be evaluated leading up to the goal
  float delta_y = (frontier_center.y-robot_pose_.position.y)/divisions;
  float delta_x = (frontier_center.x-robot_pose_.position.x)/divisions;
  // Segments at which a cell is evaluated, depends on the number of divisions
  int num_segments = ceil(frontier_dist/divisions);
  // Initialise current position of the robot that will chage a every iteration
  geometry_msgs::Point ite_point;
  ite_point.x = robot_pose_.position.x;
  ite_point.y = robot_pose_.position.y;
  ite_point.z = 0;

  if(isValidPoint(frontier_center)){
    float occupancy_value = 0.0;
    int cell_of_position;
    
    for(unsigned int i=0; i< num_segments; i++){
      // Transform current position to cell index
      cell_of_position = point2cell(ite_point);
      // With the cell index ook for the occupancy value in map_.data[]
      occupancy_value += map_.data[cell_of_position];
      // Increment the ite_point
      ite_point.x += delta_x;
      ite_point.y += delta_y;
    }

    // Now that the occupancy total value has been calculated, compute the total cost
    float total_frontier_cost = weight_frontier_size*frontier_size - 
                                weight_frontier_distance*frontier_dist - 
                                weight_occupancy_path*occupancy_value;
    ROS_INFO("COST: %f", total_frontier_cost);
    return total_frontier_cost;                        
    

  } else{
    ROS_INFO("Not a valid frontier for id: %u", frontierId);
    return -50000;
  }
}

int getHighestCostFrontierId(){
  //////////////////////////////////////////////////////////////////////
  /* getHighestCostFrontierId iterates through all frontiers and returns 
  the index of the one that has the highest cost value */
  //////////////////////////////////////////////////////////////////////

  float highest_score = -1.0;
  int highest_score_Id = -1.0;
  // Iterate for all frontiers to find which one has the highest value
  for(unsigned int i = 0; i < frontiers_.size(); i++){
    float cost = getFrontierCost(i);
    if(cost > highest_score){
      highest_score = cost;
      highest_score_Id = i;
    }
  }
  ROS_INFO("Highest score frontier: ID %d with cost %f",
  highest_score_Id, highest_score);
...
bool replan()
{
  ...
  float goal_dist = hypot(robot_pose_.position.x-target_goal_.position.x, 
  robot_pose_.position.y-target_goal_.position.y);
...
geometry_msgs::Pose decideGoal()
{
  ...
  // Return the frontier id that has the highest score since it will be the new goal
  frontierId = getHighestCostFrontierId();
  // Return the normal of the frontier that has the highest score
  float yaw = getFrontierNormal(frontierId);
  ROS_INFO("Frontier ID is %d", frontierId);

  // Define pose for the found frontier
  g.orientation = tf::createQuaternionMsgFromYaw(yaw);
  g.position.x = frontiers_[frontierId].free_center_point.x;	
  g.position.y = frontiers_[frontierId].free_center_point.y;
  
  // Calculate distance traveled
  // float traveled_dist_x = abs(prev_robot_pose_.position.x-
  // robot_pose_.position.x);
  // float traveled_dist_y = abs(prev_robot_pose_.position.y-
  // robot_pose_.position.y);
  // float traveled_dist_orient = abs(tf::getYaw(prev_robot_pose_.orientation)-
  // tf::getYaw(robot_pose_.orientation));
  // float epsilon = 0.000000000001;

  // Check if it is a valid point...
  if(!isValidPoint(g.position)){
  ...
  // } else if(traveled_dist_x<epsilon && traveled_dist_y<epsilon && traveled_dist_orient<epsilon){
  //   ROS_INFO("There are valid points, but robot is stuck. Rotating a bit...");
  //   g.orientation = tf::createQuaternionMsgFromYaw(1*M_PI);
  } else{
    g.orientation = tf::createQuaternionMsgFromYaw(yaw);
    g.position.x = frontiers_[frontierId].free_center_point.x;	
  ...
  // for(unsigned int i = 1; i<frontiers_.size(); i++){
  //   double yaw = atan2(frontiers_[i].free_center_point.y,frontiers_[i].free_center_point.x);
  //   g2.orientation = tf::createQuaternionMsgFromYaw(yaw);//2*M_PI * (rand()%100)/100.0);
  //   g2.position.x = frontiers_[i].free_center_point.x;	
  //   g2.position.y = frontiers_[i].free_center_point.y;
  //   if(frontiers_[i].size > max_frontier_size && isValidPoint(g2.position)){
  //     max_frontier_size  = frontiers_[i].size;
  //     g = g2;
